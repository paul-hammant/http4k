...
if len(sys.argv) > 1:
   if sys.argv[1] == "record":
       # TODO check that gradle/servirtium process is already started.
       url = "http://localhost:61417"
       # TODO full classpath
       gradle_process = subprocess.Popen(["gradle", "servirtium_record_mode_for_compatibility_suite"])
   elif sys.argv[1] == "playback":
       url = "http://localhost:61417"
       gradle_process = subprocess.Popen(["gradle", "servirtium_playback_mode_for_compatibility_suite"])
...
if gradle_process is not None:
    print("Killing Gradle/Servirtium")
    os.killpg(os.getpgid(gradle_process.pid), signal.SIGTERM)
    gradle_process.kill()
